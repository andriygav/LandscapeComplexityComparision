...
$\bigl|f_{\boldsymbol{\theta}^\star}(\mathbf{x}_i)-y_i\bigr|\le\varepsilon_y$ for all~$i$, then the residual-aware part of Theorem~\ref{thm:landscape-mse} and its exact identity~\eqref{eq:interp-hessian} yield the bound~\eqref{eq:muf-interp-general} on $\mu_f(f\mid D)$; under entrywise bounds this refines the worst-case $O(h^4)$ estimate (see Subsection~\ref{subsec:two-layer-comp}).
The numerical two-layer sweeps in Section~\ref{sec:experiments} instantiate this residual-aware regime (e.g.\ planted targets with small or zero label noise at~$\boldsymbol{\theta}^\star$).
\end{corollary}
...
\begin{equation}\label{eq:Rad-emp}
  ...
  \right],
\end{equation}
where $\sigma_1,\ldots,\sigma_m$ are i.i.d.\ Rademacher variables with $\mathbb{P}(\sigma_i=\pm 1)=1/2$.
The expectation $\mathbb{E}_{\boldsymbol{\sigma}}$ is conditional on the fixed sample~$D$.
The population Rademacher complexity is $\mathfrak{R}_m(\mathcal{F}):=\mathbb{E}_D\bigl[\hat{\mathfrak{R}}_m(\mathcal{F})\bigr]$~\cite{bartlett2002}.
\end{definition}
\noindent
...
Under the same input scaling, $\mu_f$ admits the upper estimate $O(n)$ by~\eqref{eq:muf-lin-mu}, while $\hat{\mathfrak{R}}_m$ in~\eqref{eq:Rad-lin-bound} is bounded by a quantity of order $W\sqrt{n/m}$; uniform bounds for the real-valued class also admit linear-in-$n$ complexity through pseudo-dimension/covering routes~\cite{bartlett2002,vapnik1998}.
...
Consider one hidden layer of width $h$:
$\mathbb{R}^{n}\xrightarrow{\mathbf{W}^{(1)}}\mathbb{R}^{h}\xrightarrow{\sigma}\mathbb{R}^{h}\xrightarrow{\mathbf{w}^{(2)}}\mathbb{R}$ with $\sigma=\mathrm{ReLU}$, bias-free weights, and
...
For a two-layer network with $|W_{jk}^{(1)\star}|\le M$ and $|w_j^{(2)\star}|\le M$,
\[
...
\begin{table}[t]
\centering
\caption{Theoretical upper-order scaling of $\mu_f$ and empirical Rademacher complexity for the linear model and the two-layer FC ReLU under squared loss (Subsections~\ref{subsec:linear-comp}--\ref{subsec:two-layer-comp}). For two-layer $\mu_f$, worst-case spectral bounds contrast with the residual-aware estimate from Theorem~\ref{thm:landscape-mse}; the latter comes from the local residual-aware analysis around~$\boldsymbol{\theta}^*$.}
...
\midrule
$\mu_f(f\mid D)$ &
$\le 2\max_i\|\mathbf{x}_i\|_2^2$ by~\eqref{eq:muf-lin-mu}; $O(n)$ when $\|\mathbf{x}_i\|_2^2=\Theta(n)$ &
worst-case: $O((hM)^4)$ (Cor.~\ref{cor:shallow}, \eqref{eq:muf-scale}, $L{=}2$). Residual-aware: if $|f_{\boldsymbol{\theta}^*}(\mathbf{x}_i)-y_i|\le\varepsilon_y$, then~\eqref{eq:muf-interp-general}; under entrywise bounds, e.g.\ $O(hM^2n^2+\varepsilon_y M_{\nabla^2 f})$ (Subsection~\ref{subsec:two-layer-comp}) \\
\addlinespace[0.35em]
$\hat{\mathfrak{R}}_m$ &
$O(W\sqrt{n/m})$ under comparable coordinates,~\eqref{eq:Rad-lin-bound} &
$O(m^{-1/2})$ in $m$ and $O(\mathrm{poly}(h)/\sqrt{m})$ in width under~\eqref{eq:Rad-nn-generic} \\
\bottomrule
\end{tabular}
\end{table}
...
The implementations are in the accompanying notebooks \texttt{code/exp01\_linear\_sweep\_n.ipynb}, \texttt{code/exp02\_linear\_sweep\_m.ipynb}, and \texttt{code/exp03\_twolayer\_sweep\_h.ipynb}. The figures are saved under \texttt{figures/}.
...
The experiment uses the entrywise-bounded aligned teacher and clustered inputs from the code (\texttt{make\_synthetic\_twolayer\_worstcase\_in\_ball}). The input dimension is tied to the width, so $n=h$. The setup uses the entrywise weight bound $M=1$, the sample size $m=512$, and hidden widths $h\in\{4,8,12,16,24,32\}$.
